    ...
    <AppShell>
      <div className="mx-auto max-w-7xl px-4 py-10">
        ...
        <div className="grid gap-4 md:grid-cols-2">
          {categories.map((c) => {
            const count = all.filter((p) => p.categoryId && p.categoryId === c.id).length;
            return (
              <Link key={c.id} to="/products" search={{ category: c.slug }} className="group relative overflow-hidden rounded-3xl border border-border bg-card p-8 shadow-glass transition hover:-translate-y-1 hover:shadow-elevated">
