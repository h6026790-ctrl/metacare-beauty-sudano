...
function AuthPage() {
    ...
    try {
      await submitFn({
        data: {
          ...
          street: mode === "register" ? form.street || null : null,
          notes: mode === "register" ? form.notes || null : null,
          state_id: mode === "register" ? form.stateId || null : null,
          city_id: mode === "register" ? form.cityId || null : null,
          neighborhood_id: mode === "register" ? form.neighborhoodId || null : null,
          request_type: mode,
        },
      });
      toast.success(
  ...
  const verify = async (e: React.FormEvent) => {
    ...
    setBusy(true);
    try {
      const res = await verifyFn({ data: { phone: form.phone, otp: form.otp } });
      const { error } = await supabase.auth.signInWithPassword({
        email: (res as any).email,
        password: (res as any).password,
      });
  ...
  const waitingMsg = lang === "ar"
    ? `مرحباً، طلبت إنشاء حساب في ميتاكير. رقمي: ${normalizePhone(form.phone)}. أرجو إرسال رمز التحقق.`
    : `Hello, I just requested a Metacare account. My phone: ${normalizePhone(form.phone)}. Please send my verification code.`;
      ...
      <div className="relative mx-auto grid min-h-[80vh] max-w-6xl items-center gap-10 px-4 py-10 md:grid-cols-2">
        <div className="hidden space-y-5 md:block">
          ...
          <p className="max-w-md text-base leading-relaxed text-muted-foreground">
            {lang === "ar"
              ? "نتحقق من كل حساب يدوياً عبر فريق خدمة العملاء لضمان تجربة آمنة وفاخرة."
              : "Every account is verified manually by Customer Service to keep the experience safe and personal."}
          </p>
          <div className="rounded-2xl border border-border bg-card/70 p-5 shadow-glass backdrop-blur">
            <p className="text-sm text-muted-foreground">
              {lang === "ar"
                ? "بعد إرسال الطلب، سيتواصل معكِ فريقنا عبر واتساب لإرسال رمز التحقق المكوّن من 6 أرقام."
                : "After you submit, our team will WhatsApp you a 6-digit verification code."}
            </p>
          </div>
        </div>
        ...
        <div className="mx-auto w-full max-w-md rounded-3xl border border-border bg-card p-6 shadow-elevated md:p-8">
          ...
          {step === "form" ? (
            <form onSubmit={sendRequest} className="space-y-3">
              ...
              <Field label={t.auth.phone} hint={lang === "ar" ? "مثال: 09xxxxxxxx" : "e.g. 09xxxxxxxx"}>
              ...
              <Field label={t.auth.whatsapp} hint={t.auth.whatsappNote}>
                <Input required type="tel" dir="ltr" placeholder="09xxxxxxxx" value={form.whatsapp} onChange={(e) => setForm({ ...form, whatsapp: e.target.value })} />
              </Field>
            ...
            <form onSubmit={verify} className="space-y-4">
              <div className="rounded-2xl border border-primary/30 bg-primary/5 p-4 text-sm">
                ...
                <p className="text-xs leading-relaxed text-muted-foreground">
                  {lang === "ar"
                    ? "سيتواصل معكِ فريق خدمة العملاء عبر واتساب لإرسال الرمز المكوّن من 6 أرقام. يمكنكِ بدء المحادثة الآن:"
                    : "Our Customer Service team will WhatsApp you the 6-digit code. You can open the chat now:"}
                </p>
                <a href={whatsappLink(METACARE_WHATSAPP, waitingMsg)} target="_blank" rel="noreferrer" className="mt-3 inline-flex items-center gap-2 rounded-full bg-success px-4 py-2 text-xs font-medium text-success-foreground">
              ...
              <button type="submit" disabled={busy} className="w-full rounded-full gradient-brand py-3 text-sm font-medium text-primary-foreground shadow-glow hover:opacity-95 disabled:opacity-60">
                {busy ? "…" : t.auth.verify}
              </button>
