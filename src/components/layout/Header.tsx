      ...
      <div className="mx-auto flex h-16 max-w-7xl items-center gap-3 px-4">
        <Sheet open={open} onOpenChange={setOpen}>
          ...
          <SheetContent side={lang === "ar" ? "right" : "left"} className="w-80">
            ...
            <nav className="mt-6 flex flex-col gap-1">
              {NAV.map((n) => (
                <Link key={n.to} to={n.to} onClick={() => setOpen(false)} className="rounded-xl px-3 py-2.5 text-sm font-medium text-foreground hover:bg-muted">
                  {t.nav[n.key]}
                </Link>
              ))}
              ...
              <div className="my-3 h-px bg-border" />
              <button onClick={() => setLang(lang === "ar" ? "en" : "ar")} className="flex items-center gap-2 rounded-xl px-3 py-2.5 text-sm font-medium hover:bg-muted">
        ...
        <div className="ms-auto flex items-center gap-1 md:ms-2">
          ...
          <Link to={user ? "/account" : "/auth"}
            className={cn("ms-1 hidden h-10 items-center gap-2 rounded-full px-4 text-sm font-medium md:flex",
